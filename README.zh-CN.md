...
前往 [open.longbridgeapp.com](https://open.longbridgeapp.com) → 开发者中心 → MCP Tokens，获取模拟交易 Token。
...
更多食谱持续更新中。有想法？[提 Issue](../../issues/new) 或阅读 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
**备用路径（headless）：** `recipe/NN/main.py` 直接调用 Longbridge Python SDK。适用于需要定时运行、CI 或无 Chat Agent 的场景。输出相同，驱动方式不同。
...
港股 / 新加坡居民：[longbridge.com](https://longbridge.com)。模拟交易方面，注册后开发者中心会自动开通沙盒环境，并同时签发 MCP Token 和 SDK 所需的 app_key / app_secret。
