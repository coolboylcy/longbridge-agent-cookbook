...
首次呼叫工具時瀏覽器自動打開 OAuth 登入。模擬交易帳戶走同一流程。透過 [Chris 的邀請連結註冊](https://longbridgeapp.com/invite/F6HEGJ)。
...
港股 / 新加坡居民：透過 [Chris 的邀請連結註冊](https://longbridgeapp.com/invite/F6HEGJ)。註冊後前往 [open.longbridge.com](https://open.longbridge.com) 開發者平台，沙盒環境自動開通。MCP 使用 OAuth 2.1 認證，無需手動取得 Token。Python SDK 備用路徑仍需使用開發者平台的 `app_key` / `app_secret` / `access_token`。
